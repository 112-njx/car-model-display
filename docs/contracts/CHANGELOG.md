...
| 0010 | 2026-09-22 | **T5** | **只增（申请）** | `APPEARANCE`（车身固定外观） | 承接 0009。T5 的 B 段已把 `VehicleModel` 接到 `carConfig` + `useCarStore`，但车身涂装/轮毂仍只能从 legacy `studioConfig` 的 `PAINTS`/`WHEELS` 取，且当前取值来自兼容 shim `useStudioStore` 的 `paint`/`finish`/`wheel` 字段——**shim 一旦按 §12.4 第 5 步删除，`VehicleModel` 就取不到外观值**。§3.2 已裁掉涂装/轮毂配置器，故外观应固化为契约常量。**建议方案**：在 `carConfig.js` 增 `export const APPEARANCE = { paint: { color, metalness, roughness }, finish: 12, wheel: { color, roughness } }`（取值取 T1 基线默认 `ivory` / `finish:12` / `turbine`），T5 改读之、删除对 `PAINTS`/`WHEELS`/shim 的三处依赖。**T5 本轮未擅自迁入**（config/** 为 T2 独占），暂以"读 shim 默认值"过渡，行为与 T1 基线一致。 | **待受理（建议 T2 或 T8 落地）** |
