...
| 38 | **点击大灯/尾灯开关车模无灯光效果（已修，c87cdbd）** | 根因：GLB 灯组最外层透明透镜材质 tembus_red.0（opacity≈0.82、带 map，前 1 个 tembus_depan、后 3 个 tembus_belakang/tembus_boot/高位刹车条 mesh 共用）在任何灯态下都把内部 emissive 发光体完全遮挡；store 不可变更新、材质名匹配、emissive 写入（前灯 #fff0cf ei=2.7、尾灯 #ff2338 ei=3.2）、HeadlightRig 光束（intensity 90）经 CDP 运行时实测全部正常。判别实验：隐藏透镜 mesh 后大灯立即点亮。修法：carConfig.MODEL_MATERIALS 新增 headlightLens/taillightLens（先判 depan 归前、其余 tembus_red 归后；紧急 bugfix 直接改 T2 独占文件，登记于此）；VehicleModel 逐 mesh clone 透镜材质并记录原始 opacity/depthWrite，帧循环按灯态驱动——前透镜开灯降 opacity 至 0.22 + depthWrite=false 透出内部 LED，后透镜红色自发光（ei 2.4、opacity 升至 0.95），关灯完全还原；悬停有轻微预览光。CDP 回归：关灯黑腔/开灯点亮/再关灯还原，前后视角均通过；verify-parts 79/79 PASS；npm run build 通过。排查中另发现 Vite HMR 在 Windows 下偶发漏收 carConfig.js 变更（内存缓存旧模块导致 .some of undefined 整页崩溃），touch 文件触发 reload 后恢复。 | 已修（待真机目验） |
