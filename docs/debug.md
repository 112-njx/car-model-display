...
| 11 | 代码冻结窗口 | T8 曾边改代码边被我验收，Vite HMR 整页刷新清空注入的 mock 与 store 状态，使断言失去意义（实测同一脚本三次运行失败集合各不相同、两次中途崩溃）。已加 `checkNoReload()` 检测并在报告里暴露。**R1 正式轮已在负责人提供的冻结窗口内跑完**（`7aead33`，`git status` 干净）。 | **已解决** |
| 12 | P1 缺陷回流 T8：拖走相机后点「复位」无反应 | 我最初的"端到端确证"测试无效（点击落在视口外，见 T9-06 的更正），但**缺陷本身是真的**（T7 在 CHANGELOG 0011 有独立证据）。T8 已受理并落地 0019（`applyCameraView` + 令牌），**回归通过**。 | **已解决** |
| 13 | **P1 缺陷回流 T8：待机自转「交互即停」不满足语音与程序化路径** | 实测语音指令「打开车窗」正确执行但自转继续（`autoRotating` true→true）；每 2s 调 `store.bumpInteraction()` 持续 16s 仍会在约 10s 后进入自转 ⇒ `IdleAutoRotate` 不消费 §13.2 冻结的 `store.lastInteractionAt`。真实指针路径正常。影响人工项 **A14**，**阻塞 S3**（P1 未清零）。根因与建议修法见 `docs/qa-report.md` §4.1。 | **待处理（需 T8 修复）** |
| 14 | 手机真机验收时间窗 | 需负责人安排手机 Chrome + 同一 Wi-Fi（开发机 WLAN `10.14.6.9`），按 `qa-checklist.md` §3.2 B1–B13 逐项勾选。AI 无法代做。B11 真机语音须等 T10b 的 https URL。 | 待处理（需约定时间窗） |
